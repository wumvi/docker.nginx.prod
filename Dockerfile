...
LABEL image.version="1.1" nginx.version="1.13.9" openssl.version="1.0.2n"  run.mode="prod"
...
RUN DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get --no-install-recommends -qq -y install wget curl apt-transport-https lsb-release ca-certificates zlib1g-dev unzip cmake build-essential vim libboost-all-dev git gettext-base && \
    mkdir -p /www/ && \
    mkdir -p /www/conf/ && \
    mkdir -p /soft/ && \
    #
...
HEALTHCHECK --interval=1m --timeout=3s CMD STATUS=`curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8383/ping` && [ $STATUS = 200 ]
